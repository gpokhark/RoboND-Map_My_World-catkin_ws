...
bool handle_drive_request(ball_chaser::DriveToTarget::Request &req,
  ...
  ROS_INFO("DriveToTarget received - j1:%1.2f, j2:%1.2f", (float)req.linear_x,
           (float)req.angular_z);

  // Publish the linear_x and angular_z to the motor_command_publisher
  ...
  res.msg_feedback =
      "drive_bot handle_drive_request publish(linear_x: " +
      std::to_string((double)motor_command.linear.x) +
      ", angluar_z: " + std::to_string((double)motor_command.angular.z) + ")";
